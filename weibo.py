...
class Client(object):
    ...
    def shareWeibo(self, status, pic=None, redirect_uri=None):
        if redirect_uri:
            status = '{} {}'.format(status, redirect_uri)

        if pic:
            resp = self.post('statuses/share', status=status, pic=pic)
        else:
            resp = self.post('statuses/share', status=status)
        return resp
